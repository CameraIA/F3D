		...
		repeat=0;
	 	while(repeat<nrepeats)
	 	{
			...
			if(bothRuns==1){
				...
				run("F3D Image Processing (JOCL)", "options=[{input : "+origTitle+" , intermediateSteps : false , enableZorder : false , devices : [{ Device: 0 , MaxNumSlices: "+NSlices+" } { Device: 1 , MaxNumSlices: "+NSlices+" } ] , useVirtualStack : false , virtualDirectory : empty , filters : [{ Name : MMFilterEro , Mask : [{maskImage : "+structElemWindow+"}] }]}]");     //No virtual stack
